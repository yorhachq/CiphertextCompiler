...
int main(void)
{
	SetConsoleTitle(L"CiphertextCompiler");
...
//	system("mode con cols=110 lines=30");//若设置窗口大小，当输出内容过长时，将无法完整显示（窗口内无法上下滚动）
	...
	if (check == 1)////////////////////////////////////////////////////////////////////////////////////////////加密部分
	{
		...
		if (LEN > 4093) 
		{
			printf("已超过允许输入的最大串长，继续加密将导致部分内容丢失，是否仍继续(Y/N)\n");//受到windows控制台缓冲区大小的限制，当输入4094个字符后将无法继续输入
			do //对用户的异常行为进行应对
			{
				switch (getch())
				{
				...
				case 'n':
					...
					get = getchar(); 
					exit(0);
				default:system("cls"); printf("无效输入,请选择是否继续加密(Y/N)\n"); check = 0;
				}
			} while (check == 0);
			check = 0;
		}
